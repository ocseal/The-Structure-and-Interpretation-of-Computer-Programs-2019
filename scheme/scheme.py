...
def scheme_eval(expr, env, _=None): # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in SPECIAL_FORMS:
        ...
        if check_procedure(operator) is None:
            return scheme_apply(operator, rest.map(lambda x: scheme_eval(x, env)), env)
        if isinstance(operator, Pair):
            return Pair(scheme_eval(operator, env))
        else:
            raise SchemeError('Invalid operator')
        # END PROBLEM 5
...
class Frame(object):
    ...
    def make_child_frame(self, formals, vals):
        ...
        if len(vals) != len(formals):
            raise SchemeError('Quantity disagreement')
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        # BEGIN PROBLEM 6
        special = scheme_eval(expressions.second.first, env)
        env.define(target, special)
        return target
        # END PROBLEM 6
...
def do_lambda_form(expressions, env):
    ...
    lamproc = LambdaProcedure(formals, expressions.second, env)
    return lamproc
    # END PROBLEM 9
...
def make_let_frame(bindings, env):
    ...
    while bindings is not nil:
        ...
        params = Pair(scheme_eval(bind.second.first, env), params)
        check_formals(formals)
        frame = env.make_child_frame(formals, params)

...
def best_predictor(user, restaurants, feature_fns):
    ...
    # BEGIN Question 8
    predictor = [find_predictor(user, reviewed, function) for function in feature_fns]
    pred_r2 = {p: r2 for p, r2 in predictor}
    return max(pred_r2, key=lambda p: pred_r2[p])
    # END Question 8
...
def rate_all(user, restaurants, feature_fns):
    ...
    for r in restaurants:
        if r in reviewed:
            temp = {restaurant_name(r): user_rating(user, restaurant_name(r))}
            dict.update(temp)
        else:
            temp = {restaurant_name(r): predictor(r)}
            dict.update(temp)
    return dict
    # END Question 9

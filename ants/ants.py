...
class Place(object):
    ...
    def add_insect(self, insect):
        ...
        if insect.is_ant:
            ...
            elif base_ant.can_contain(second_ant):
                base_ant.contain_ant(insect)

            elif second_ant.can_contain(base_ant) is True and base_ant.is_container is False:
                second_ant.contain_ant(base_ant)
                self.ant = second_ant
    ...
    def remove_insect(self, insect):
        ...
        if insect.is_ant:
            ...
            # BEGIN Problem 13
            if type(insect) is QueenAnt and insect.imposter == False:
                return
            else:
            # END Problem 13
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        ...
        bee_range = 0
        temp_location = self.place

        while temp_location is not hive:
            if random_or_none(temp_location.bees) is not None:
                if self.min_range <= bee_range <= self.max_range:
                    return random_or_none(temp_location.bees)
            temp_location = temp_location.entrance
            bee_range += 1
...
# END Problem 13
    ...
    def action(self, colony):
        ...
        if self.imposter:
            ...
            temp_location = self.place.exit
            while temp_location is not None:
                ant = temp_location.ant
                if ant is not None:
                    if ant not in QueenAnt.doubled_ants:
                        if ant.is_container and ant.contained_ant is not None:
                            if ant.contained_ant not in QueenAnt.doubled_ants:
                                ant.contained_ant.damage *= 2
                                QueenAnt.doubled_ants.append(ant.contained_ant)
                        ant.damage *= 2
                        QueenAnt.doubled_ants.append(ant)
                    if ant.is_container:
                        if ant.contained_ant is not None and ant.contained_ant not in QueenAnt.doubled_ants:
                            ant.contained_ant.damage *= 2
                            QueenAnt.doubled_ants.append(ant.contained_ant)
                temp_location = temp_location.exit
